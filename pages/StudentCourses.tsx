...
const StudentCourses: React.FC<StudentCoursesProps> = ({ onNavigate }) => {
  ...
  // Use local data directly instead of fetching from backend
  const courses = COURSES.map((course, index) => ({
    ...course,
    progress: course.progress !== undefined ? course.progress : (index === 3 ? 0 : 100),
    totalLessons: course.totalLessons || 34,
    completedLessons: course.completedLessons || (course.progress === 100 ? 34 : 0),
  }));
