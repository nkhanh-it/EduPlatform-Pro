...
const Courses: React.FC<CoursesProps> = ({ onNavigate }) => {
  ...
  useEffect(() => {
    const fetchCourses = async () => {
      try {
        let q = collection(db, 'courses') as any;
        ...
        if (!snapshot.empty) {
          let fetchedCourses = snapshot.docs.map(doc => ({ id: String(doc.id), ...doc.data() as any } as Course));
          if (search.trim()) {
      ...
      <div className="flex-1 flex flex-col h-full overflow-hidden relative">
        ...
        <main className="flex-1 overflow-y-auto p-6 md:p-8">
          <div className="max-w-7xl mx-auto space-y-8">
            ...
            {/* Course Grid */}
            {loading ? (
              <div className="py-12 text-center text-slate-500">Đang tải khóa học...</div>
            ) : courses.length === 0 ? (
              <div className="py-12 text-center text-slate-500">Không tìm thấy khóa học phù hợp.</div>
            ) : (
