...
const AdminRevenue: React.FC<AdminRevenueProps> = ({ onNavigate }) => {
   ...
   useEffect(() => {
      const fetchAll = async () => {
         // Using mock data for revenue dashboard since backend endpoints are replaced
         setStats({
            totalRevenue: 0,
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                     <div className="bg-gradient-to-br from-blue-600 to-blue-700 rounded-2xl p-6 text-white shadow-lg shadow-blue-500/20">
                        ...
                        <h3 className="text-3xl font-bold mb-4">
                           {stats?.totalRevenue ? `${stats.totalRevenue.toLocaleString('vi-VN')} đ` : '--'}
                        </h3>
                     ...
                     <div className="bg-white dark:bg-dark-card rounded-2xl p-6 border border-gray-200 dark:border-dark-border shadow-sm">
                        <div className="flex justify-between items-start mb-4">
                           ...
                           <span className="flex items-center text-green-500 text-sm font-bold bg-green-50 dark:bg-green-900/10 px-2 py-0.5 rounded"><ArrowUpRight size={16} /> {stats?.successTransactions ?? 0}</span>
                        </div>
                        <p className="text-slate-500 dark:text-slate-400 text-sm">Giao dịch thành công</p>
                        <h3 className="text-2xl font-bold text-slate-900 dark:text-white">{stats?.successTransactions ?? 0}</h3>
                     </div>
                     <div className="bg-white dark:bg-dark-card rounded-2xl p-6 border border-gray-200 dark:border-dark-border shadow-sm">
                        <div className="flex justify-between items-start mb-4">
                           ...
                           <span className="flex items-center text-red-500 text-sm font-bold bg-red-50 dark:bg-red-900/10 px-2 py-0.5 rounded"><ArrowDownRight size={16} /> {stats?.pendingTransactions ?? 0}</span>
                        </div>
                        <p className="text-slate-500 dark:text-slate-400 text-sm">Hoàn tiền / Hủy</p>
                        <h3 className="text-2xl font-bold text-slate-900 dark:text-white">{stats?.pendingTransactions ?? 0}</h3>
                     </div>
                  </div>
                  ...
                  <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                     ...
                     <div className="bg-white dark:bg-dark-card p-6 rounded-2xl border border-gray-200 dark:border-dark-border shadow-sm">
                        ...
                        <div className="h-[300px]">
                           <ResponsiveContainer width="100%" height="100%">
                              <BarChart data={[
                                 { name: 'Thành công', val: stats?.successTransactions ?? 0 },
                                 { name: 'Đang chờ', val: stats?.pendingTransactions ?? 0 },
                              ]}>
                                 <CartesianGrid strokeDasharray="3 3" vertical={false} stroke="#e2e8f0" />
                                 <XAxis dataKey="name" axisLine={false} tickLine={false} tick={{ fill: '#64748b' }} dy={10} />
                     ...
                     <div className="overflow-x-auto">
                        <table className="w-full text-left border-collapse">
                           ...
                           <tbody className="divide-y divide-gray-100 dark:divide-dark-border text-sm">
                              {transactions.map((tx) => (
                                 <tr key={tx.id} className="hover:bg-gray-50 dark:hover:bg-dark-border/30 transition-colors">
                                    <td className="p-4 font-mono text-xs text-slate-500">#{tx.id}</td>
                                    <td className="p-4 font-medium">{tx.user?.name}</td>
                                    <td className="p-4 text-slate-600 dark:text-slate-300">{tx.courseTitle}</td>
                                    <td className="p-4 text-slate-500">{tx.date}</td>
                                    <td className="p-4 font-bold text-slate-900 dark:text-white">{tx.amount.toLocaleString('vi-VN')}đ</td>
