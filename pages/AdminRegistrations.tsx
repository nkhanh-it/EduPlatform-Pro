...
const AdminRegistrations: React.FC<AdminRegistrationsProps> = ({ onNavigate }) => {
  // Mock data for registrations based on existing constants
  const registrations = STUDENTS.map((student, index) => ({
      ...
      amount: COURSES[index % COURSES.length].price
  }));
                             ...
                             <tr key={reg.id} className="group hover:bg-gray-50 dark:hover:bg-dark-border/30 transition-colors">
                                ...
                                <td className="p-4">
                                   <div className="flex items-center justify-end gap-2">
                                      {reg.status === 'pending' && (
                                         <>
                                            <button className="p-1.5 text-green-600 hover:bg-green-50 dark:hover:bg-green-900/20 rounded-lg transition-colors" title="Duyệt">
                                               <CheckCircle size={18} />
                                            </button>
                                            <button className="p-1.5 text-red-600 hover:bg-red-50 dark:hover:bg-red-900/20 rounded-lg transition-colors" title="Từ chối">
                                               <XCircle size={18} />
                                            </button>
                                         </>
                                      )}
