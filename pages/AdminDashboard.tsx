...
import { AdminDashboardStats } from '../apiTypes';
...
const AdminDashboard: React.FC<AdminDashboardProps> = ({ onNavigate }) => {
   ...
   useEffect(() => {
      const fetchDashboard = async () => {
         // Calculate real stats from Firestore if possible, else mix real & mock
         try {
            // 1. Get real students count from Firestore
            ...
            <div className="flex-1 overflow-y-auto p-8">
               <div className="max-w-7xl mx-auto flex flex-col gap-8">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-4 gap-6">
                     {[
                        { label: 'Tổng doanh thu', value: data?.totalRevenue ? `${data.totalRevenue.toLocaleString('vi-VN')} đ` : '--', change: data?.revenueChange ?? '', icon: DollarSign, color: 'text-green-500', bg: 'bg-green-500/10' },
                        { label: 'Tổng học viên', value: data?.totalStudents ?? '--', change: data?.studentChange ?? '', icon: Users, color: 'text-blue-500', bg: 'bg-blue-500/10' },
                        { label: 'Số khóa học', value: data?.totalCourses ?? '--', change: '', icon: Award, color: 'text-purple-500', bg: 'bg-purple-500/10' },
                        { label: 'Đăng ký', value: data?.totalEnrollments ?? '--', change: '', icon: PlayCircle, color: 'text-orange-500', bg: 'bg-orange-500/10' },
                     ].map((stat, i) => (
                           ...
                           <tbody className="divide-y divide-gray-100 dark:divide-dark-border">
                              {([] as any[]).map((item) => (
                                 <tr key={item} className="hover:bg-gray-50 dark:hover:bg-dark-border/30 transition-colors">
                                    ...
                                    <td className="px-6 py-4 text-sm">{user?.name || 'Học viên'}</td>
